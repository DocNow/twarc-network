...
def test_min_subgraph():
    result = runner.invoke(network, ['test-data/tweets.jsonl', '--format', 'json', '--min-subgraph-size', '4'])
    graph = json.loads(result.output)
    assert len(graph['nodes']) == 285

def test_max_subgraph():
    result = runner.invoke(network, ['test-data/tweets.jsonl', '--format', 'json', '--max-subgraph-size', '15'])
    graph = json.loads(result.output)
...
def test_tweets():
    result = runner.invoke(network, ['test-data/tweets.jsonl', '--format', 'json', '--nodes', 'tweets'])
    assert result.exit_code == 0
    graph = json.loads(result.output)
    assert len(graph['nodes']) == 613

def test_hashtags():
    result = runner.invoke(network, ['test-data/tweets.jsonl', '--format', 'json', '--nodes', 'hashtags'])
    assert result.exit_code == 0
    graph = json.loads(result.output)
